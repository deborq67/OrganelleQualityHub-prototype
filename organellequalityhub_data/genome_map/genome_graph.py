...
def classify_regions(ir_features, genome_size):
    ...
    """
    candidates = [f for f in ir_features
                  if abs(int(f.location.end) - int(f.location.start)) >= 1000]
    if len(candidates) < 2:
        return []

    irs = sorted(
        sorted(candidates, key=lambda f: abs(int(f.location.end) - int(f.location.start)),
               reverse=True)[:2],
...
def generate_map(gb_path, ir_data=None):
    ...
    region_features = [f for flist in features_all.values()
                       for f in flist if f.type == 'repeat_region']
    ...
    for flist in features.values():
        for f in flist:
            name = f.qualifiers.get('gene', [''])[0] or f.qualifiers.get('product', [''])[0]
            ...
            if name:
                mid = (s + e) / 2
                cds_f_track.text(f'{name}', mid, size=4) if f.location.strand >= 0 \
    ...
    for flist in gbk.get_seqid2features(feature_type='rRNA').values():
        for f in flist:
            rrna_track.rect(int(f.location.start), int(f.location.end), fc=COL_RRNA, ec='none')
    ...
    for flist in gbk.get_seqid2features(feature_type='tRNA').values():
        for f in flist:
            trna_track.rect(int(f.location.start), int(f.location.end), fc=COL_TRNA, ec='none')
