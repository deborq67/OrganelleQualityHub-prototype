...
def run(options, stdout, style):
    ...
    if not to_process:
        raise PipelineError('None of the pending accessions have a matching GenBank file.')

    if options.get('limit'):
        to_process = to_process[:options['limit']]

    stdout.write(f'{len(to_process)} row(s) to backfill '
                 f'({missing_files} pending row(s) have no matching .gb file).')

    n_workers = options.get('workers') or DEFAULT_WORKERS
    chunksize = options.get('chunksize') or max(1, min(DEFAULT_CHUNKSIZE, len(to_process) // (n_workers * 4)))
