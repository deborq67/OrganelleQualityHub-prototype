"""Processes all GenBank files in plastid_files/ and saves parsed IR reports to IR_Identification.

Extracted from pipelines/management/commands/ir_setup.py so the command
...
def _flush(rows, update, stdout, style):
    ...
    stdout.write(style.SUCCESS(f'  wrote {len(records)} entries'))
    return len(records)


def run(options, stdout, style):
    """Parses new/changed GenBank files into IR_Identification rows. Returns (written, verb)."""
    from apps.inverted_repeats.models import IR_Identification
    ...
    stdout.write(f'{len(file_list)} new files to process.')
    ...
    with Pool(processes=n_workers) as pool:
        for result in pool.imap_unordered(parse_file, file_list, chunksize=chunksize):
            ...
            if len(batch) >= BATCH_SIZE:
                written += _flush(batch, options["update"], stdout, style)
                batch = []
